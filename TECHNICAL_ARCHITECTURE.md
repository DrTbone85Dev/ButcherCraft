...
| `com.butchercraft.machine` | Grinder, Patty Former, Bandsaw, packaging station, base machine block entities, tick helpers. |
...
| `com.butchercraft.world.workforce.employee` | Pure Employee Identity, Employment Records, lifecycle, shift references, presence observation, entity linkage records, employee record persistence, and the bounded historical IM-027 state. Runtime employee entities consume these records for movement and non-authoritative operation/carry presentation only. |
...
| `com.butchercraft.world.workforce.machineoperation` | Pure Workforce-owned finite machine-operation assignment identity, exact employee and Workstation Instance bindings, lifecycle, completed-quantity observation, typed failures, deterministic manager, and schema-versioned persistence. It stores owner references but no inventory, custody, Machine Run registry, Scheduler work, or mutation authority. |
...
| `com.butchercraft.world.execution` | Pure generic Execution runtime foundation for operation identity, immutable authorization evidence consumption, lifecycle, attempts, handler boundary, owner result evidence, Unknown Outcome state, Scheduler handler integration, and versioned operation persistence. IM-012 registers the Grinder handler, IM-018 registers the Patty Former handler, IM-031A through IM-031C add Execution-owned persistent Machine Runs for both powered machines, and IM-032B lets Workforce submit identity-bound finite START/STOP intent through that same boundary. Execution retains Run and child-admission authority. |
...
- `com.butchercraft.world.inventory.freshness` owns Inventory Freshness Identity primitives and live scoped freshness representation for Transaction validation. It does not define a global Inventory revision, mutate Inventory, publish checkpoints, or persist new schema.
- `com.butchercraft.world.transaction.binding` owns validation binding primitives used by live `TransactionManager`, `TransactionValidator`, `TransactionExecutor`, replay, duplicate/conflict classification, and Transaction result evidence. It does not persist reusable authority, migrate durable result evidence, change Scheduler dispatch, change Planning cycles, implement Execution or Allocation, or publish checkpoints.
...
- The Patty Former consumes the same workstation framework with only the `butchercraft:patty_forming` capability. Ground Beef to Beef Patties is selected through product/species/profile/operation definitions and transformation data, not Patty Former product branches. IM-028C separates input readiness from operation authority. Accepted IM-031C and IM-032B permit explicit player control or a finite Workforce assignment to request canonical Execution-owned Machine Run START/STOP; Scheduler dispatches bounded children and Workstation retains recipe, inventory, and owner-result authority. Production does not control Machine Runs. Server ticks and reload never infer START or RESUME authorization from input validity.
...
- Employee navigation quality is Workforce-owned runtime behavior. Employee entities reconstruct transient destinations from durable department, transfer-assignment, machine-operation-assignment, and reservation state; path nodes remain transient. IM-032B supersedes normal new IM-027 requests with one finite persistent assignment for an exact employee and exact Grinder or Patty Former instance. The Workforce coordinator observes owner evidence and calls canonical reservation and Machine Run services; it does not own inventory, custody, Runs, Scheduler dispatch, or Production.
- Workforce-owned Workstation reservation persistence at `<world>/butchercraft/workstation_reservations.json` uses schema 2 for exact reservation identity, role, Workstation Instance generation, employee, request, transfer/assignment endpoint scope, lifecycle evidence, persisted operating position, and invalidation evidence. It admits at most one operator plus one compatible transfer-bound handler per exact instance and at most one reservation per employee. Conservative schema-1 migration never infers `MACHINE_OPERATOR`. A reservation is eligibility evidence only: it is not a pathfinding engine and grants no endpoint mutation, custody, Machine Run, Scheduler, or Execution authority.
...
- Economic Production defines executable operational intent and lifecycle without owning Goods, time, Inventory quantities, Business Runtime, Workforce, Orders, or transaction history. Scheduler-backed economic completion requires one APPLIED Production Transaction. The IM-016 promoted Grinder path is separately completed from Grinder owner result evidence plus Execution result evidence, without economic Inventory mutation. IM-018 adds one narrow Grinder to Patty Former chain observation path with explicit manual-transfer state and product-flow identity validation; Production still does not move ItemStacks or own workstation mutation.
...
- Workforce definitions currently persist in schema-versioned JSON at `<world>/butchercraft/workforce_definitions.json`; departments and employees persist separately in `departments.json` and `employee_records.json`. Transfer assignments persist at `employee_material_handling_assignments.json`, role-aware reservations at `workstation_reservations.json`, and finite machine-operation assignments at `employee_machine_operation_assignments.json`. The machine-operation document stores exact employee, Workstation Instance, policy, finite target, proven completed count, lifecycle, reservation, Run, pending-supply, freshness, and failure references only. It never stores Machine Run, Workstation inventory, Scheduler work, or Material Handling custody state.
...
- Production Process definitions, Plan definitions, and Run runtime state persist independently at `<world>/butchercraft/production_processes.json`, `production_plans.json`, and `production_runs.json`. Run persistence may reference workstation assignments, ordered chain steps, Execution Operation Identities, workstation owner result evidence, Execution result evidence, and Production-owned completion evidence; it does not duplicate workstation or Execution state. All three candidates are cross-validated before publication; per-file replacement does not claim a three-file filesystem transaction.
- Economic Planning persists terminal Observations, Needs, Opportunities, Candidates, Approved Plans, Constraints, resolution/submission runtime, reports, and cadence state at `<world>/butchercraft/planning_observations.json`, `planning_needs.json`, `planning_opportunities.json`, `planning_candidates.json`, `planning_approved_plans.json`, `planning_runtime.json`, and `planning_cadence.json`. An existing artifact set must contain all six legacy artifact files; cadence state is versioned and legacy six-file saves load without synthetic trigger evidence. Complete cycle structure, cadence evidence, and external authority references are validated before publication.
- Generic Execution persists schema-1 operation runtime at `<world>/butchercraft/execution_operations.json`. The file records immutable authorization evidence, operation lifecycle, attempts, owner result evidence, terminal result evidence, and typed failures. Live runtime authorization consumption tokens are not persisted. DG-003 classifies handler-registry evolution through exact Handler Contract Identities and immutable historical profiles; unknown or incompatible registries block authority, while unchanged additive-compatible startup preserves historical persistence without rewriting it.
...
- Employee employment state and department assignment belong in Workforce-owned employee records. Employee entities persist only a stable Employee Identity link and movement anchor; they are not the authoritative employment, department, reservation, or navigation-recovery record. Future skill systems require a separate approved owner or explicit Workforce record extension.
...
IM-004 adds `com.butchercraft.world.transaction.binding` and `com.butchercraft.world.inventory.freshness` as support for the approved Transaction Validation Authority ADR. These packages provide Proposal Identity, Inventory Freshness Identity, Validation Plan Identity, Validation Consumption Authority, duplicate/conflict classification, and result-evidence binding primitives.

IM-008 integrates those primitives with live Transaction submission, execution,
replay, duplicate/conflict behavior, and Production observation. Live accepted
validations bind the exact proposal, scoped Inventory freshness, and immutable
validation plan before Inventory mutation. Public validation evidence alone
does not grant mutation authority. Transaction persistence remains schema 1 and
does not yet persist durable binding/result evidence.
...
`SimulationSchedulerService` initializes after `OrderContractService`, executes after the Simulation Clock's post-tick listener, and persists schema-2 state at `<world>/butchercraft/simulation_scheduler.json`. Unknown persisted Work types, mismatched clock ticks, persisted `RUNNING` state, invalid effect declarations, and unsupported effect-policy schema fail visibly. Phase 20 installs `butchercraft:production_run`; Phase 21 installs `butchercraft:economic_planning_cycle`; IM-011 installs `butchercraft:generic_execution_operation` before scheduler loading. IM-012 connects one grinder workstation handler through that generic Work type. Allocation integration and public runtime registration remain unimplemented. See `docs/SIMULATION_SCHEDULER.md` for schemas, lifecycle, ordering, invariants, measured scale, and limitations.
...
schema-2 persistence content. Coordinated restoration validates that the Clock
...
Completion constructs one ordered explicit transaction change plan: all consumed inputs first, preserving Inventory entry metadata, followed by every produced output. `TransactionValidator` validates the complete candidate state and `TransactionExecutor` commits the batch atomically through `InventoryManager`. The Run becomes completed only after authoritative Transaction history records `APPLIED` and Transaction result evidence is available for Scheduler observation; rejection or commit failure leaves no partial Inventory mutation.

`ProductionService` initializes after Goods, Actors, Inventory, Transactions, Orders/Contracts, Business Runtime, and Workforce are available. It loads Process, Plan, and Run files as one cross-validated candidate, installs the Production handler before scheduler loading, then validates persisted Work references after the scheduler is active. IM-016 adds a Grinder-side adapter that records Production-owned assignment and completion evidence from existing Grinder and Execution observations. IM-018 adds Patty Former-side observation and one ordered Grinder to Patty Former chain model with explicit manual-transfer state. See `docs/PRODUCTION_FRAMEWORK.md` for schemas, transition rules, requirements, persistence, examples, invariants, measured scale, and extension points.
...
`EconomicPlanningService` installs its handler before Scheduler load, initializes after Production and Scheduler binding, loads Planning artifacts and cadence state, and ensures one cadence-gated continuation Work exists in the PLANNING stage. Interrupted cycles, malformed or partial files, invalid provenance graphs, conflicting trigger identities, active in-progress cadence state, and missing external references fail visibly. See `docs/ECONOMIC_PLANNING_ENGINE.md` for the schemas, pipeline, ownership boundaries, persistence, invariants, measured scale, and deferred scope.
...
operation through the existing workstation and Execution boundaries. IM-028B
adds one explicit Cutting Table-output-to-Grinder-input Beef Trim assignment
while Material Handling retains custody authority and Workstation retains
endpoint mutation authority. The Cutting Table has one Beef Short Loin input,
one T-Bone Steak primary output, and one dedicated Beef Trim output. Exactly
one player-operated recipe runs through Workstation, Execution, and Scheduler;
...
exact Ground Beef stack and adds the explicit operation gate. IM-029 connects
that destination to a Workstation-owned Grinder output endpoint through the
existing Workforce assignment and Material Handling custody protocols. Deposit
commits only endpoint custody and `READY` state; it does not issue Execution
authority, Scheduler work, or employee operation authority.
...
- The entity persists only the Employee Identity link and movement anchor;
  navigation and operation/carry presentation remain transient projections.
- Workforce transfer assignment intent persists separately in
  `<world>/butchercraft/employee_material_handling_assignments.json`; it never
  contains an ItemStack.
- The tracked carry snapshot contains only transfer reference, one display
  ItemStack, display state, and monotonic observation revision. It derives from
  proven Material Handling custody and rejects stale or conflicting revisions.
...
- The permission-gated `/butchercraft employee operate <employee> <x> <y> <z>
  <quantity>` command creates or observes one finite persistent assignment for
  an exact Grinder or Patty Former. `operate-status` is read-only and
  `operate-cancel` requests safe exact-Run cancellation. Arrival, reservation,
  or material alone remains passive.
- The permission-gated `/butchercraft employee transfer <employee> <source-x>
  <source-y> <source-z> <destination-x> <destination-y> <destination-z>` command
  binds explicit same-dimension endpoints. `transfer-status` observes the
  assignment and `transfer-cancel` requests safe pre-custody cancellation or
  a physical source return through Material Handling.
- Startup restores World Identity, Workstation instance and journal state,
  Workstation projections, Material Handling, employee records, reservations,
  transfer assignments, and machine-operation assignments. Cross-owner
  coherence is validated before authority publication; active Runs remain
  `RESTART_REQUIRED` until explicit player action.
...
Additional employee recipes or transfer routes, autonomous RESUME, job
claiming, Production-driven workflows, multi-machine tending, batch hauling,
skill gain, scheduling, productivity, and payroll remain separately gated.
