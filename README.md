...
Registered content is limited to development fixtures used to verify registration, assets, data generation, diagnostics, ItemStack product data integration, data-driven processing definitions, the temporary workstation framework, and the current Grinder, Bandsaw, Packaging Table, retail product foundation, and Packaging Supplies. Milestones 1B and 1C add pure Java engine and processing-framework foundations only; Milestone 1D connects product snapshots to ItemStacks; Milestone 2A adds server datapack definitions and a processing graph; Milestone 2B adds a development-only Processing Workstation block; Milestone 2D proves the Grinder can process Beef, Pork, and Bison Trim to matching ground products through data-driven definitions; Milestone 2E proves ordered multi-output fabrication through the Bandsaw; version 0.6.0 begins the pure Java Material Transformation Engine foundation; version 0.6.1 connects the Grinder to capability-based transformation execution; version 0.6.2 adds the immutable transformation registry; version 0.6.3 formalizes the canonical transformation definition schema; version 0.6.4 adds the canonical product definition foundation; version 0.6.5 adds the pure Java transformation serialization contract; version 0.6.6 adds pure Java atomic multi-output transformation transactions; version 0.6.7 migrates the Bandsaw proof to registry-driven atomic transformation execution; version 0.6.8 loads transformation definitions from datapack JSON resources; version 0.6.9 loads product definitions from datapack JSON and activates products and transformations as one validated content snapshot; version 0.7.0 expands the datapack-backed Bandsaw proof with a multi-stage beef fabrication chain; version 0.8.0 adds the Packaging Table workstation foundation, Retail Product Framework, Packaging Supplies, packaging execution proof, and asset framework; version 0.9.0-alpha.1 adds World Identity region and deterministic naming foundations without gameplay interactions.
...
Planning and architecture documents live at the repository root. Treat `PROJECT_IDENTITY.md`, `PROJECT_RULES.md`, and accepted decisions in `DECISIONS.md` as authoritative. The pure domain engine is documented in `docs/BUTCHERCRAFT_ENGINE.md`, the processing framework is documented in `docs/PROCESSING_FRAMEWORK.md`, the Material Transformation Engine is documented in `docs/MATERIAL_TRANSFORMATION_ENGINE.md`, transformation serialization is documented in `docs/TRANSFORMATION_SERIALIZATION.md`, product datapack loading is documented in `docs/DATAPACK_PRODUCTS.md`, transformation datapack loading is documented in `docs/DATAPACK_TRANSFORMATIONS.md`, retail product definitions are documented in `docs/RETAIL_PRODUCT_FRAMEWORK.md`, packaging supplies are documented in `docs/PACKAGING_SUPPLIES.md`, atomic multi-output transformations are documented in `docs/MULTI_OUTPUT_TRANSFORMATIONS.md`, the ItemStack product data bridge is documented in `docs/PRODUCT_DATA_INTEGRATION.md`, datapack-backed definitions are documented in `docs/PRODUCT_AND_PROCESSING_DEFINITIONS.md`, multi-output processing is documented in `docs/MULTI_OUTPUT_PROCESSING.md`, the workstation framework is documented in `docs/WORKSTATION_FRAMEWORK.md`, the Grinder proof is documented in `docs/GRINDER.md`, the Bandsaw proof is documented in `docs/BANDSAW.md`, the Bandsaw transformation migration is documented in `docs/BANDSAW_TRANSFORMATION_MIGRATION.md`, the v0.7.0 beef fabrication expansion is documented in `docs/BEEF_FABRICATION_EXPANSION.md`, and the v0.8.0 Packaging Table foundation is documented in `docs/PACKAGING_TABLE.md`.
