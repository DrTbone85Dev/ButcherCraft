...
public final class SimulationClockCheckpointSnapshotRestorer implements CheckpointOwnerSnapshotRestorer {
    private final SimulationConfiguration configuration;
    private final SimulationEventBus eventBus;
    private final Supplier<SimulationClock> currentClock;
    private final Consumer<SimulationClock> publisher;

    public SimulationClockCheckpointSnapshotRestorer(
            SimulationConfiguration configuration,
            SimulationEventBus eventBus,
            Supplier<SimulationClock> currentClock,
            Consumer<SimulationClock> publisher
    ) {
        this.configuration = Objects.requireNonNull(configuration, "configuration");
        this.eventBus = Objects.requireNonNull(eventBus, "eventBus");
        this.currentClock = Objects.requireNonNull(currentClock, "currentClock");
    ...
    public CheckpointOwnerRestorationPreparation prepare(CheckpointOwnerRestorationRequest request) {
        ...
        if (!ownerId().equals(request.descriptor().ownerId())) {
            return failed(CheckpointFailureCode.OWNER_RESTORATION_VALIDATION_FAILURE,
                    "ownerId", "Clock restorer received a snapshot for a different owner");
        }
        if (request.descriptor().snapshotSchemaVersion()
            ...
            return failed(CheckpointFailureCode.UNSUPPORTED_OWNER_SNAPSHOT_SCHEMA,
                    "snapshotSchemaVersion", "Clock checkpoint snapshot schema is unsupported");
        }
        ...
        if (!digest.equals(request.descriptor().contentDigest())) {
            return failed(CheckpointFailureCode.PAYLOAD_DIGEST_MISMATCH,
                    "contentDigest", "Clock checkpoint payload digest does not match descriptor");
        }
        if (!request.selectedGenerationId().equals(request.descriptor().generationId())) {
            return failed(CheckpointFailureCode.MIXED_GENERATION_IDENTITY,
                    "generationId", "Clock snapshot belongs to a different checkpoint generation");
        }
        if (!request.worldIdentityRoot().equals(request.descriptor().worldIdentityRoot())) {
            return failed(CheckpointFailureCode.WORLD_IDENTITY_MISMATCH,
                    "worldIdentityRoot", "Clock snapshot belongs to a different World Identity root");
        }
        try {
            SimulationClockCheckpointSnapshotCodec.ParsedClockSnapshot parsed =
                    SimulationClockCheckpointSnapshotCodec.deserialize(request.payloadBytes(), configuration);
            if (!parsed.configurationIdentity().equals(request.descriptor().configurationIdentity())) {
                return failed(CheckpointFailureCode.PLATFORM_DETERMINISM_MANIFEST_MISMATCH,
                        "configurationIdentity", "Clock configuration identity does not match descriptor");
            }
            if (parsed.state().simulationTick() != request.descriptor().representedSimulationTick()) {
                return failed(CheckpointFailureCode.OWNER_RESTORATION_VALIDATION_FAILURE,
                        "simulationTick", "Clock payload tick does not match descriptor tick");
            }
            SimulationClock restoredClock = new SimulationClock(configuration, parsed.state(), eventBus);
            CheckpointOwnerValidationMetadata metadata = new CheckpointOwnerValidationMetadata(
                    ...
                    Map.of(
                            ...
                            Long.toString(restoredClock.simulationTick()),
        ...
        } catch (SimulationClockCheckpointSnapshotCodec.UnsupportedSnapshotSchemaException exception) {
            return failed(CheckpointFailureCode.UNSUPPORTED_OWNER_SNAPSHOT_SCHEMA,
                    "snapshotSchemaVersion", "Clock checkpoint snapshot schema is unsupported");
        } catch (RuntimeException exception) {
            return failed(CheckpointFailureCode.OWNER_RESTORATION_VALIDATION_FAILURE,
                    "payload", "Clock checkpoint snapshot could not be restored");
        }
    }
    ...
    private record ClockRestorationCandidate(
            SimulationClock restoredClock,
        ...
        private ClockRestorationCandidate {
            restoredClock = Objects.requireNonNull(restoredClock, "restoredClock");
        ...
        public CheckpointOwnerRestorationPublicationResult publish() {
            try {
                publisher.accept(restoredClock);
                ...
                return CheckpointOwnerRestorationPublicationResult.failed(ownerId(), List.of(failure(
                        ...
                        ownerId().value(),
                        "Clock owner failed to publish restored state"
                )));
            }
        }
        ...
        public CheckpointOwnerRestorationPublicationResult rollbackPublication() {
            try {
                publisher.accept(previousClock);
                return CheckpointOwnerRestorationPublicationResult.published(ownerId());
            } catch (RuntimeException exception) {
                return CheckpointOwnerRestorationPublicationResult.failed(ownerId(), List.of(failure(
                        CheckpointFailureCode.OWNER_PUBLICATION_FAILURE,
                        ownerId().value(),
                        "Clock owner failed to roll back restored state"
                )));
            }
        }
    }
