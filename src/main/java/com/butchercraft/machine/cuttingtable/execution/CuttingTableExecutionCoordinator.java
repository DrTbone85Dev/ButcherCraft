...
public final class CuttingTableExecutionCoordinator implements WorkstationExecutionCoordinator {
    public static final CuttingTableExecutionCoordinator INSTANCE = new CuttingTableExecutionCoordinator();

    private CuttingTableExecutionCoordinator() {
    }

    @Override
    public WorkstationExecutionStartResult start(WorkstationExecutionStartRequest request) {
        Objects.requireNonNull(request, "request");
        if (!CuttingTableExecutionConstants.AUTHORIZED_OPERATIONS.contains(request.operation().operationId())) {
            return WorkstationExecutionStartResult.rejected(WorkstationFailure.of(
                    ...
                    "Cutting Table Execution authorizes only the accepted fabrication recipe"
            ));
        }
        if (request.operation().definition().operation().workstationCapability()
                .filter(CuttingTableWorkstation.CAPABILITY_ID::equals)
                .isEmpty()) {
            return WorkstationExecutionStartResult.rejected(WorkstationFailure.of(
                    WorkstationFailureCode.NO_COMPATIBLE_OPERATION,
                    request.operation().operationId(),
                    "Cutting Table Execution requires the Cutting Table capability"
            ));
        }
        try {
            ...
            String operationIdentity = CuttingTableExecutionIdentities.operationIdentity(request.operation());
            String frozenInputIdentity = CuttingTableExecutionIdentities.inputIdentity(request.frozenInputs());
            String expectedOutputIdentity =
                    CuttingTableExecutionIdentities.expectedOutputIdentity(request.expectedOutputs());
            String sourceFreshnessIdentity = CuttingTableExecutionIdentities.sourceFreshnessIdentity(
                    workstationIdentity,
                    request.operation(),
                    operationIdentity,
                    frozenInputIdentity,
            ...
            ExecutionAuthorizationEvidence evidence = ExecutionAuthorizationEvidence.issued(
                    CuttingTableExecutionConstants.OWNER_SUBSYSTEM_ID,
                    CuttingTableExecutionConstants.EXECUTABLE_REFERENCE_TYPE,
                    workstationIdentity,
            ...
            return WorkstationExecutionStartResult.rejected(WorkstationFailure.of(
                    ...
                    exception.getMessage() == null
                            ? "Cutting Table Execution authorization failed"
        ...
        try {
            ...
            if (!prepared.accepted()) {
                return WorkstationExecutionDispatchResult.rejected(WorkstationFailure.of(
                        ...
                        first(prepared.messages(), prepared.failureCode()
                                .orElse(ExecutionFailureCode.UNKNOWN).serializedName())
                ));
            }
            ...
            return WorkstationExecutionDispatchResult.rejected(WorkstationFailure.of(
                    WorkstationFailureCode.EXECUTION_DISPATCH_REJECTED,
                    first(submitted.messages(), submitted.failureCode()
                            .orElse(WorkFailureCode.UNKNOWN).serializedName())
            ));
        } catch (RuntimeException exception) {
            return WorkstationExecutionDispatchResult.rejected(WorkstationFailure.of(
                    WorkstationFailureCode.EXECUTION_DISPATCH_REJECTED,
                    exception.getMessage() == null
                            ? "Cutting Table Execution dispatch failed"
                            : exception.getMessage()
            ));
        }
    }
    ...
    public WorkstationExecutionCancelResult cancel(WorkstationExecutionCancelRequest request) {
        ...
        try {
            ...
            if (runtime.isPresent() && !runtime.orElseThrow().status().isTerminal()) {
                ...
                if (!cancelled.successful()) {
                    return WorkstationExecutionCancelResult.rejected(WorkstationFailure.of(
                            WorkstationFailureCode.EXECUTION_DISPATCH_REJECTED,
                            first(cancelled.messages(), cancelled.failureCode()
                                    .orElse(WorkFailureCode.UNKNOWN).serializedName())
                    ));
                }
            }
            ...
            return WorkstationExecutionCancelResult.rejected(WorkstationFailure.of(
                    WorkstationFailureCode.EXECUTION_DISPATCH_REJECTED,
                    exception.getMessage() == null
                            ? "Cutting Table Execution cancellation failed"
                            : exception.getMessage()
            ));
        }
    }
