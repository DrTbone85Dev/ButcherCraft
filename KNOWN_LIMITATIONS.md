...
- Product, packaging, and transformation datapack loaders validate the current proof content, including packaging supply references, but schema migrations, datapack-driven category catalogs, full fabrication catalogs, packaging recipes, and dynamic product-to-item creation rules are not implemented yet.
- The product definition registry is pure Java and currently covers Grinder, Bandsaw, and one retail packaged-product proof. It does not replace ItemStack product data, datapack-backed processing product definitions, product-to-item mappings, storage rules, spoilage, packaging execution, or quality expansion.
- The Packaging Table foundation is intentionally inventory-only. It does not resolve operations, evaluate transformations, consume inputs, create outputs, mutate packaging state, affect quality, or participate in order fulfillment.
