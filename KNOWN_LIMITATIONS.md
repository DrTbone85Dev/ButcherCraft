...
- Version 0.6.0 begins the pure Java Material Transformation Engine foundation. Version 0.6.1 migrates only the Grinder to capability-based transformation execution. Version 0.6.2 adds an immutable transformation registry and registers built-in Grinder transformations in Java. Version 0.6.3 formalizes the canonical transformation definition schema. Version 0.6.4 adds a pure Java product definition foundation. Version 0.6.5 adds a pure Java transformation serialization contract. Version 0.6.6 adds pure Java atomic multi-output transformation transactions. Version 0.6.7 migrates only the Bandsaw proof to registry-driven atomic transformation execution. It does not migrate datapack registries, smoker behavior, packaging behavior, cooler behavior, menus, screens, or product data components.
...
- The transformation registry is immutable and authoritative for current transformation lookup, but its built-in definitions are still Java-defined. Datapack transformation loading is deferred.
- The transformation schema is validated and has a pure Java serialization contract, but JSON resource discovery, datapack loading, reload listeners, implemented migrations, external metadata policy expansion, and datapack error-reporting are not implemented yet.
- The product definition registry is pure Java and currently covers only the Grinder and Bandsaw proof products. It does not replace ItemStack product data, datapack-backed processing product definitions, product-to-item mappings, storage rules, spoilage, packaging, or quality expansion.
