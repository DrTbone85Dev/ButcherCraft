...
- Phase 19 adds a deterministic persistent Work scheduler and bounded six-stage pipeline. Phase 20 installs one internal Production Run handler; no other live handlers or automatic Work creation exist. Scheduler schema 1 still has no automatic catch-up, partial-tick resume, persistence migration, interrupted-Work recovery, global rollback for non-transactional handler side effects, public handler API, audit UI, logistics, markets, population, pricing, automatic Order/Contract behavior, or gameplay.
...
- The World Simulation Clock persists infrastructure time state in `<world>/butchercraft/simulation_state.json` and business runtime state persists at `<world>/butchercraft/business_runtime.json`. The Work scheduler can now advance explicitly registered Production Runs, but it does not create Plans or schedule deliveries, inspections, maintenance gameplay, refrigeration, economy, reputation, workers, UI, commands, or gameplay events.
