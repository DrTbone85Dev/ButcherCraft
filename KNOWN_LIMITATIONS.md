...
- The transformation registry is immutable and authoritative for current transformation lookup, and its current built-in definitions load from datapack JSON. Implemented schema migrations, external metadata policy expansion, and broader user-facing datapack diagnostics are not implemented yet.
- The transformation schema is validated and has a pure Java serialization contract, but product definition datapack loading into the pure product registry is not implemented yet.
