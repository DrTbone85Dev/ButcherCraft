...
- IM-031A provides persistent Machine Run and Workstation operating-state infrastructure; IM-031B activates `POWERED_CONTINUOUS_EXPLICIT_STOP` for the player-controlled Grinder, and IM-031C activates the same machine-neutral coordination for the player-controlled Patty Former. Both machines retain workstation-specific recipe and inventory authority, and each cycle remains separately bounded. The Cutting Table remains discrete, employee Grinder operation remains one bounded request, and employee Patty Former operation is unavailable. Automatic restart, forced chunk loading, wear, damage, lubrication, maintenance, breakdown, employee START/STOP, and Production machine control remain gated.
