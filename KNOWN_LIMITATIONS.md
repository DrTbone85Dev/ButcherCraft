...
- Workforce definitions describe positions, staffing requirements, qualifications, and shift assignments. Department records describe Workforce-owned department identities and anchors. Employee records describe individual employment state, department assignment, and a linked in-world employee representation. IM-027 permits one reservation-scoped Beef Trim Grinder request and IM-028B permits one explicit Cutting Table-to-Grinder Beef Trim transfer with visible carrying. Workforce still does not provide job claiming, employee Patty Former operation or Ground Beef transport, additional transported materials or quantities, employee inventory, automatic workstation selection, hiring markets, payroll, productivity, Production-driven assignment, general Logistics, or autonomous workflows.
- IM-028B Material Handling is intentionally limited to one employee, one explicitly selected same-dimension Cutting Table Beef Trim output, one explicitly selected Grinder destination, and exactly one Beef Trim. The Cutting Table supports only one player-operated Beef Short Loin to T-Bone Steak plus Beef Trim recipe; broader fabrication remains gated. The flow does not transport Ground Beef, search for machines, run queues, force-load chunks, start processing automatically, mutate economic Inventory, or expose a public logistics API. Unknown Outcome remains operator-gated and proven unresolved custody remains fail-visible.
- IM-028C makes one Patty Former Ground Beef destination structurally valid and adds a player explicit-operation gate. It does not add a Material Handling request, Workforce assignment, employee carry view, Grinder source selection, employee Patty Former operation, autonomous transfer, or IM-029. The READY gate is intentionally Patty Former-specific; other shared processing workstations retain their existing start policies until separately authorized.
