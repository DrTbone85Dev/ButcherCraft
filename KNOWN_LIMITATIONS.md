...
- IM-031A provides persistent Machine Run and Workstation operating-state infrastructure, and IM-031B activates `POWERED_CONTINUOUS_EXPLICIT_STOP` for the player-controlled Grinder only. The Patty Former still uses explicit one-cycle requests, the Cutting Table remains discrete, and employee Grinder operation remains one bounded request rather than Run control. Automatic restart, forced chunk loading, wear, damage, maintenance, employee START/STOP, and Production machine control remain gated.
