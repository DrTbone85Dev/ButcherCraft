...
- The active content snapshot is immutable and authoritative for current product and transformation lookup, and its current built-in definitions load from datapack JSON. Implemented schema migrations, external metadata policy expansion, and broader user-facing datapack diagnostics are not implemented yet.
- Product and transformation datapack loaders validate the current proof content, but schema migrations, datapack-driven category catalogs, broader fabrication catalogs, and dynamic product-to-item creation rules are not implemented yet.
