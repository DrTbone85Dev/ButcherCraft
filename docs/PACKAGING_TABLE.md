...
Status: v0.8.0 Project Meat Counter workstation foundation with Sprint E asset framework

## Purpose

Version 0.8.0 adds the Packaging Table as a permanent ButcherCraft workstation. It validates registration, placement, menus, inventory persistence, client screen wiring, item-handler exposure, and block-break recovery. Sprint 2 adds retail product data definitions, Sprint C adds physical packaging supply items, Sprint D connects the table to the existing processing workstation framework for the first packaged retail flow, and Sprint E prepares the visual asset contract for later final artwork.
...
- GUI texture contract at `butchercraft:textures/gui/packaging_table.png`
...
Placeholder assets are abstract Minecraft-style development assets and are expected to be replaced later. The active model, texture, and GUI handoff details are documented in `docs/ASSET_MANIFEST.md`, `docs/ASSET_SPECIFICATIONS.md`, and `docs/PACKAGING_TABLE_GUI_SPEC.md`.
...
The Packaging Table block model is generated by `ButcherCraftBlockStateProvider`. The current model stays inside normal block bounds and separates surface, frame, and roll texture references:

- `butchercraft:block/workstation/packaging_table_surface`
- `butchercraft:block/workstation/packaging_table_frame`
- `butchercraft:block/workstation/packaging_table_roll`

The block item inherits the generated block model. The screen draws a stable 176x166 active region from `butchercraft:textures/gui/packaging_table.png` on a 256x256 texture canvas, then renders functional slot and progress overlays from synchronized menu state.
