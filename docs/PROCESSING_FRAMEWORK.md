...
Version 0.6.6 adds atomic transformation transactions. Version 0.6.7 migrates only the Bandsaw to the atomic transformation execution path. Version 0.6.8 loads the current Grinder and Bandsaw transformation definitions from datapack JSON. Version 0.6.9 loads the current product definitions from datapack JSON and validates transformations against the candidate product registry before activating both registries together. Existing processing-operation resolver and workstation controller behavior remain unchanged.
