...
- Keep `com.butchercraft.world.allocation` pure Java and independent of Planning, Production, Scheduler, Inventory, Transactions, Minecraft, and NeoForge. M22A defines immutable structural artifacts; M22B permits deterministic `AllocationSetRuntime` lifecycle mutation only through `AllocationRuntimeService` and exposes immutable registries, views, history, queries, and report structures. Do not add algorithms, Capacity ledgers, providers, persistence, Scheduler stage 350, or integration gates without a later owner-authorized milestone.
