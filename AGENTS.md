...
- Keep `com.butchercraft.world.allocation` pure Java and independent of Planning, Production, Scheduler, Inventory, Transactions, Minecraft, and NeoForge. M22A defines immutable structural artifacts; M22B permits deterministic `AllocationSetRuntime` lifecycle mutation only through `AllocationRuntimeService`; M22C permits the explicit-input deterministic Cycle, detached temporary Capacity ledger, first-fit complete-Set evaluation, and atomic candidate-state publication; M22D permits explicit immutable provider descriptors and registry, read-only provider adapters, canonical sequential observation, typed failure isolation, and immutable observation bundles. Authoritative mutation must remain confined to Cycle publication. Do not add concrete providers, persistence, Scheduler stage 350, Planning handoff, Production gates, automatic Cycle invocation, or gameplay integration without a later owner-authorized milestone.
